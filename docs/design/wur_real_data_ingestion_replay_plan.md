...
- 2024 plant/pot identity across spacing changes: the time series and imagery carry no plant identity at all (compartment-level only); per-plant data exists only in `Harvest.xlsx` (destructive harvests on three dates and the final harvest, plants numbered `<compartment>-<n>`).

Image archives, inspected 2026-09-14 from their zip central directories (execution steps in `wur_execution_plan.md`):

- **2024 canopy (31.4 GB, 7404 PNG, stored almost uncompressed).** One directory per camera: `cam_1` = 3.06 Reference, `cam_18` = 3.08 Trigger, `cam_19` = 3.07 IDEAS, `cam_20` = 3.03 Agrifusion, `cam_21` = 3.02 MuGrow, `camir_27` = 3.01 Tomatonuts. Filenames `<cam>_YYYY_MM_DD_hh_mm_ss_<rgb|depth|irR|irL>.png` in local time (CET/CEST), four captures a day at 04, 08, 12 and 16 h, 2024-09-03 to 2024-11-21. Each camera is an Oak-D S2 POE about 1.5 m above the crop facing down; `configs/<cam>.json` gives intrinsics for the 3840x2160 colour frame (depth is aligned to colour). No extrinsics or measured pose exist.
- **2023 canopy (4.5 GB, 952 PNG).** One camera (`cam_19`), `images/<date>/{rgb,depth}/`, 2023-09-05 to 2023-11-09 with a gap 2023-10-28 to 10-31; `canopy_camera_config.json` intrinsics.
- **2023 single-plant (7.5 GB, 1916 PNG).** `train` / `validation1-3` / `test` splits, each with `rgb/`, `depth/`, `ground_truth_<split>.json` (height, `fw_plant`, `leaf_area`, `number_of_red_fruits` per anonymised image id) and `anon_keys_<split>.json` mapping ids back to the original `camir_27_<timestamp>_<treatment>` filenames; `oak-d-s2-poe_conf.json` intrinsics and the challenge's technical PDF.
- **4TU access.** Download URLs honour HTTP `Range` (206), so single members can be read without the archive, but bursts of small range requests are throttled (503). Mirror once, build snapshots from the mirror.
