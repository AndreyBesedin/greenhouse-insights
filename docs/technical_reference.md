...
Profiles: `tiny` (compartment 3.06, tabular only), `dev` (all six compartments, tabular only), `full` (also the ~43 GB image archives). Both datasets have adapters: 2024 for its full tabular data, 2023 so far for its climate workbook (weekly crop measurements and destructive harvests are the next steps). Perception, replay-time recommendations and backtesting are later phases of the plan.
